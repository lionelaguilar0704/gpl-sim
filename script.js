...
function newPlayer(){return{
 ...
 berries:500,bounty:0,fruit:"None",fruitType:"",fruitMastery:0,fruitSkills:[],
...
function addFeed(t,major=false){p.feed.unshift({age:p.age,text:t,major});p.feed=p.feed.slice(0,80)}
function addLog(t){p.log.unshift(`Age ${p.age}: ${t}`);p.log=p.log.slice(0,160)}
function addNews(headline,body){p.newspapers.unshift({age:p.age,headline,body});p.newspapers=p.newspapers.slice(0,35);addFeed(`Newspaper: ${headline}`,true)}
...
function checkConquerorAwakening(force){
 ...
 if(dramatic&&score>=16&&Math.random()<chance){
   ...
   major("CONQUEROR'S HAKI AWAKENED — the air cracked under your will.");
...
function gainHakiXP(type,n){
 ...
 const levelKey=type;
 const threshold=(p[levelKey]+1)*3;
 if(xp>=threshold && p[levelKey]<tree.length){
   p[levelKey]++; const skill=tree[p[levelKey]-1]; 
...
function useObservation(){
 ...
 if(p.observation<=0){notice("Observation Dormant","You have not awakened Observation Haki yet.");return}
 const actions=["sensed an ambush before it formed","noticed someone lying","tracked a hidden enemy","felt a Devil Fruit user nearby","predicted a minor danger"];
 silent(`Observation Haki: ${pick(actions)}.`);
 apply({observationXP:1,intelligence:1});
 showMenu();
}
function useArmament(){
 if(!spendAction())return;
 if(p.armament<=0){notice("Armament Dormant","You have not awakened Armament Haki yet.");return}
 const actions=["hardened your fists against steel","coated your weapon briefly","blocked a bullet with willpower","trained impact against stone","damaged a slippery power user"];
 silent(`Armament Haki: ${pick(actions)}.`);
 apply({armamentXP:1,strength:1,health:-3});
 showMenu();
}
function useConqueror(){
 if(!spendAction())return;
 if(p.conqueror<=0){notice("Conqueror Dormant","You do not currently have Conqueror's Haki. King-like actions raise the chance.");return}
 const actions=["weak enemies stepped back without knowing why","your crew stood taller under your presence","a room went silent when you entered","your glare stopped a fight before it began"];
 major(`Conqueror's Haki: ${pick(actions)}.`);
 apply({conquerorXP:1,presence:1,leadership:1});
 showMenu();
}

function fruitRoll(){if(p.fruit!=="None"){silent("Found a Devil Fruit, but already had powers.");return}
 if(Math.random()<.58){let f=pick(DATA.fruits);p.fruit=f.name;p.fruitType=f.type;p.fruitMastery=1;p.fruitSkills=[f.tree[0]];major(`Ate the ${f.name}: ${f.power}. Unlocked ${f.tree[0]}.`);addNews("STRANGE FRUIT INCIDENT",`${p.name} reportedly gained unnatural powers after eating a mysterious fruit.`)}
...
function passiveYear(){
 ...
 if(income>0)silent(`Collected business income: ฿${fmt(income)}.`);
...
function ageUp(){p.age++;passiveYear();major("A year passed.");if(p.age===16&&p.path==="Undecided")choosePath();else if(Math.random()<.35)showEvent(chooseWeightedEvent());else showMenu()}
function simulateCrewLives(){
 p.crew.forEach(c=>{
   if(Math.random()<.18){c.loyalty=clamp(c.loyalty+pick([-1,0,1]),0,10)}
   if(Math.random()<.08){c.traits.push(pick(["Scarred","Inspired","Restless","Afraid","Devoted","Suspicious"]));silent(`${c.name} changed: ${c.traits[c.traits.length-1]}.`)}
   if(Math.random()<.05 && p.age>20){c.haki=pick(["Observation","Armament"]);major(`${c.name} awakened ${c.haki} Haki.`)}
 });
}
function simulateWorld(){
 if(Math.random()<.45){let ev=pick(DATA.worldEvents);p.timeline.unshift({age:p.age,text:ev});p.timeline=p.timeline.slice(0,50);silent(`World: ${ev}.`)}
 if(Math.random()<.12 && p.rivals.length){let r=pick(p.rivals);silent(`Rumor: your rival, ${r}, grew stronger somewhere at sea.`)}
}
...
function combatScore(mode="duel"){
 let score=p.strength*2+p.speed+p.durability+p.sword*2+p.marksmanship*1.5+p.armament*5+p.observation*3+p.conqueror*6+p.fruitMastery*3+p.crew.length*1.5;
...
function resolveCombat(kind,targetPower,reward){
 ...
 if(score>=enemy){apply(reward||{});silent(`Won ${kind} encounter. Score ${Math.round(score)} vs ${Math.round(enemy)}.`);return true}
 let dmg=Math.floor((enemy-score)/2)+10;p.health-=dmg;p.bounty+=kind==="naval"?8000:3000;major(`Lost or barely escaped ${kind} encounter. Took ${dmg} damage.`);if(Math.random()<.35)injuryRoll();if(p.health<=0)survivalCheck();return false;
}
...
{title:"Animal in Chains",tags:["chains","freedom"],text:"You find an animal trapped behind a tavern. It is scared, angry, and hurt.",choices:[["Free and heal it",{honor:2,companionRoll:1,charisma:1,freedom:1,mercy:1},"Freed an animal from chains."],["Ignore it",{discipline:1},"Ignored the trapped animal."],["Use it as bait during a fight",{infamy:2,honor:-2,ruthless:2},"Used a trapped animal as bait."],["Track whoever chained it",{sneak:1,mystery:1,freedom:1},"Tracked the people who chained the animal."]]},
...
{title:"Rival Appears",tags:["rival","fighter"],text:"A future rival crosses your path. They recognize your potential and hate it immediately.",choices:[["Challenge them",{strength:1,bounty:5000,rivalRoll:1,heat:1,ambition:1,courage:1},"Challenged a future rival."],["Talk them down",{charisma:2,rivalRoll:1,leadership:1},"Talked down a future rival."],["Study their weakness",{intelligence:2,rivalRoll:1,observationXP:1},"Studied a rival's weakness."],["Avoid them",{sneak:1},"Avoided a dangerous rival."]]},
...
{title:"Emperor Scout",tags:["newworld","fame","rival"],text:"A scout from a terrifying New World power offers you protection in exchange for loyalty.",choices:[["Refuse proudly",{honor:2,bounty:30000,rivalRoll:1,defiance:3,independence:2},"Refused an emperor scout."],["Accept temporarily",{infamy:1,berries:20000},"Accepted an ugly alliance."],["Attack the scout",{strength:2,armamentXP:2,conquerorXP:2,bounty:50000,health:-20,courage:2,ambition:1},"Attacked an emperor scout."],["Gather intel",{sneak:2,mystery:1,observationXP:1},"Gathered intel on an emperor crew."]]},
{title:"Poneglyph Rubbing",tags:["secret","government"],text:"You find an ancient rubbing hidden inside a sealed wall. Reading it could invite disaster.",choices:[["Study it carefully",{intelligence:2,mystery:2,heat:2,ambition:1},"Studied a forbidden rubbing."],["Sell it to the underworld",{berries:25000,infamy:2,heat:3,greed:2},"Sold a forbidden historical clue."],["Destroy it",{discipline:1,honor:-1},"Destroyed a dangerous clue."],["Hide it on your ship",{mystery:1,sneak:1,heat:1},"Hid the rubbing away."]]},
{title:"Sea Beast Attack",tags:["sea","fighter"],text:"A sea beast rises beside your vessel, large enough to crack the hull in one bite.",choices:[["Fight it head-on",{strength:2,bounty:5000,shipDamage:20,health:-10,courage:2,armamentXP:1},"Fought off a sea beast."],["Outmaneuver it",{navigation:2,shipXP:1,observationXP:1},"Outmaneuvered a sea beast."],["Sacrifice cargo",{berries:-3000},"Sacrificed cargo to escape a sea beast."],["Use your crew as bait",{infamy:3,honor:-3,crewLoss:1,ruthless:3},"Used someone as bait."]]}
];
...
function setup(){
 p=newPlayer();
 $("screen").innerHTML=`<h2>Start a New Life</h2><p><b>v3.0 Living World Engine</b> adds silent BitLife-style feeds, all three Haki types, action-based Conqueror awakening, crew lives, world timeline, hidden traits, destiny, island memory, and smoother actions.</p><input id="nameInput" placeholder="Character name, or leave blank for random"><div class="choices"><button class="primary" onclick="randomStart()">Random Life</button><button onclick="chooseOriginScreen()">Choose Origin</button>${localStorage.getItem("gpls_save_v30")?'<button onclick="loadGame()">Load Saved Life</button>':''}<button class="danger" onclick="clearSave()">Clear Save</button></div>`;
 render();
}
...
function showMenu(){
 ...
 $("screen").innerHTML=`<h2>Age ${p.age}: ${p.name}'s Life</h2><p><b>${p.actionsLeft}</b> energy left. Most actions now update the feed silently. Major story moments still interrupt.</p>
 <div class="menuGrid">
...
function travelRegion(i){if(!spendAction())return;let r=DATA.regions[i];let ok=p.navigation+crewBonus("navigator")*2>=r.req.nav && p.ship.maxHp>=r.req.ship;if(!ok){p.health-=10;damageShip(20);major(`Failed to safely reach ${r.name}. Need Navigation ${r.req.nav} and ship HP ${r.req.ship}.`);showMenu();return}p.region=r.name;let islands=DATA.islands.filter(x=>x.region===r.name);p.island=pick(islands).name;major(`Reached ${r.name}. ${r.desc}`);addNews("NEW SEA REACHED",`${p.name} has entered ${r.name}.`);showMenu()}
...
function duelRival(){if(!spendAction())return;if(!p.rivals.length)gainRival();let power=30+currentRegion().danger*10+p.rivals.length*3;let win=resolveCombat("duel",power,{bounty:12000,infamy:1,armamentXP:1,conquerorXP:1,courage:1});if(win&&Math.random()<.35){let r=p.rivals.shift();major(`Defeated rival permanently: ${r}.`);p.legacy+=2}showMenu()}
function navalBattle(){if(!spendAction())return;if(p.ship.name==="None"){notice("No Ship","You need a ship for naval battles.");return}let power=30+currentRegion().danger*12;resolveCombat("naval",power,{berries:15000,bounty:18000,shipXP:2,infamy:1,leadership:1});damageShip(Math.floor(Math.random()*15));showMenu()}
function raidTarget(){if(!spendAction())return;let power=35+currentRegion().danger*10;resolveCombat("duel",power,{berries:22000,bounty:25000,infamy:2,heat:2,ruthless:1});showMenu()}
...
function smuggleRun(){if(!spendAction())return;let power=25+currentRegion().danger*8;let win=resolveCombat("escape",power,{berries:25000,infamy:1,heat:2});if(win)silent("Completed smuggling run.");showMenu()}
...
function render(){
 ...
 localStorage.setItem("gpls_save_v30",JSON.stringify(p));
...
function showTab(tab,silentRender=false){
 ...
 if(tab==="world")html=`<h3>World Map</h3><p>${currentRegion().desc}</p><div class="cardGrid">${DATA.regions.map(r=>`<div class="miniCard"><h4>${r.name}</h4><p>Danger ${r.danger}/10 · Req Nav ${r.req.nav}, Ship HP ${r.req.ship}</p></div>`).join("")}</div><h3>Current Island</h3><p><b>${currentIsland().name}</b> — ${currentIsland().type}: ${currentIsland().desc}</p><h3>World Timeline</h3>${p.timeline.map(t=>`<div class="feedEntry">Age ${t.age}: ${t.text}</div>`).join("")||"<p>The world is quiet... for now.</p>"}<h3>Newspapers</h3>${p.newspapers.length?p.newspapers.map(n=>`<div class="newspaper"><h2>${n.headline}</h2><p><b>Age ${n.age}</b> — ${n.body}</p></div>`).join(""):"<p>No newspapers yet.</p>"}`;
...
function help(){ $("screen").innerHTML=`<h2>How to Play v3.0</h2><div class="notice"><b>Silent Feed:</b> Most actions no longer interrupt. Check the Feed tab for results.<br><br><b>Haki:</b> Observation, Armament, and Conqueror are separate. Use Haki actions once awakened.<br><br><b>Conqueror:</b> Not pure RNG. Leadership, ambition, defiance, courage, presence, sacrifice, and independence raise your chance.<br><br><b>Living World:</b> Crew change, rivals grow, islands remember you, and world events continue each year.</div><div class="choices"><button class="primary" onclick="showMenu()">Back</button></div>`}
function manualSave(){localStorage.setItem("gpls_save_v30",JSON.stringify(p));silent("Game saved.");showMenu()}
function loadGame(){p=JSON.parse(localStorage.getItem("gpls_save_v30"));showMenu()}
function clearSave(){localStorage.removeItem("gpls_save_v30");setup()}
